!< FURY definition of weber unit.
module fury_unit_weber
!-----------------------------------------------------------------------------------------------------------------------------------
!< FURY definition of weber unit.
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(unit_magnetic_flux) :: unit_weber
  !< Definition of weber unit.
  contains
    ! public overrided methods
    procedure, pass(self) :: set !< set the unit.
endtype unit_weber

interface unit_weber
  !< Ovearloading unit_weber name with a creator function.
...
contains
  ...
  !< @todo Load from file.
  !---------------------------------------------------------------------------------------------------------------------------------
  class(unit_weber), intent(inout)         :: self           !< The unit.
  ...
  self%symbol = 'Wb' ; if (present(symbol)) self%symbol = symbol
  self%dimensionality = '[mass]*[length]*[length]/[time]/[time]/[current]'
  if (present(dimensionality)) self%dimensionality = dimensionality
  error_ = 0
  if (present(error)) error = error_
  !---------------------------------------------------------------------------------------------------------------------------------
  endsubroutine set
endmodule fury_unit_weber

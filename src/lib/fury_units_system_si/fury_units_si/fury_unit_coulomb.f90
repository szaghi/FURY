!< FURY definition of coulomb unit.
module fury_unit_coulomb
!-----------------------------------------------------------------------------------------------------------------------------------
!< FURY definition of coulomb unit.
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(unit_electric_charge) :: unit_coulomb
  !< Definition of coulomb unit.
  contains
    ! public overrided methods
    procedure, pass(self) :: set !< set the unit.
endtype unit_coulomb

interface unit_coulomb
  !< Ovearloading unit_coulomb name with a creator function.
...
contains
  ...
  type(unit_coulomb)                 :: unit         !< The unit.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  !< @todo Load from file.
  !---------------------------------------------------------------------------------------------------------------------------------
  class(unit_coulomb), intent(inout)         :: self           !< The unit.
  ...
  self%symbol = 'C' ; if (present(symbol)) self%symbol = symbol
  self%dimensionality = '[time]*[current]' ; if (present(dimensionality)) self%dimensionality = dimensionality
  error_ = 0
  if (present(error)) error = error_
  !---------------------------------------------------------------------------------------------------------------------------------
  endsubroutine set
endmodule fury_unit_coulomb

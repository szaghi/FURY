!< FURY definition of ohm unit.
module fury_unit_ohm
!-----------------------------------------------------------------------------------------------------------------------------------
!< FURY definition of ohm unit.
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(unit_electric_resistance) :: unit_ohm
  !< Definition of ohm unit.
  contains
...
interface unit_ohm
  !< Ovearloading unit_ohm name with a creator function.
...
contains
  ...
  !< @todo Load from file.
  !---------------------------------------------------------------------------------------------------------------------------------
  class(unit_ohm), intent(inout)         :: self           !< The unit.
  ...
  self%symbol = 'ohm' ; if (present(symbol)) self%symbol = symbol
  self%dimensionality = '[mass]*[length]*[length]/[time]/[time]/[time]/[current]/[current]'
  if (present(dimensionality)) self%dimensionality = dimensionality
  error_ = 0
  if (present(error)) error = error_
  !---------------------------------------------------------------------------------------------------------------------------------
  endsubroutine set
endmodule fury_unit_ohm

!< FURY definition of radian unit.
module fury_unit_radian
!-----------------------------------------------------------------------------------------------------------------------------------
!< FURY definition of radian unit.
!-----------------------------------------------------------------------------------------------------------------------------------
use fury_unit_angle
...
!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(unit_angle) :: unit_radian
  !< Definition of radian unit.
  contains
    ! public overrided methods
    procedure, pass(self) :: set !< set the unit.
endtype unit_radian

interface unit_radian
  !< Ovearloading unit_radian name with a creator function.
...
contains
  ...
  type(unit_radian)                  :: unit           !< The unit.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  !< @todo Load from file.
  !---------------------------------------------------------------------------------------------------------------------------------
  class(unit_radian), intent(inout)         :: self           !< The unit.
  ...
  self%symbol = 'rad' ; if (present(symbol)) self%symbol = symbol
  self%dimensionality = self%dimensionality_ ; if (present(dimensionality)) self%dimensionality = dimensionality
  error_ = 0
  if (present(error)) error = error_
  !---------------------------------------------------------------------------------------------------------------------------------
  endsubroutine set
endmodule fury_unit_radian

!< FURY definition of lumen unit.
module fury_unit_lumen
!-----------------------------------------------------------------------------------------------------------------------------------
!< FURY definition of lumen unit.
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(unit_luminous_flux) :: unit_lumen
  !< Definition of lumen unit.
  contains
    ! public overrided methods
    procedure, pass(self) :: set !< set the unit.
endtype unit_lumen

interface unit_lumen
  !< Ovearloading unit_lumen name with a creator function.
...
contains
  ...
  !< @todo Load from file.
  !---------------------------------------------------------------------------------------------------------------------------------
  class(unit_lumen), intent(inout)         :: self           !< The unit.
  ...
  self%symbol = 'lm' ; if (present(symbol)) self%symbol = symbol
  self%dimensionality = '[luminosity]' ; if (present(dimensionality)) self%dimensionality = dimensionality
  error_ = 0
  if (present(error)) error = error_
  !---------------------------------------------------------------------------------------------------------------------------------
  endsubroutine set
endmodule fury_unit_lumen

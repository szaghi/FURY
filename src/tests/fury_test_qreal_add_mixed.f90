...
call force%unset
force = force1 + force2
test_passed(1) = force%stringify(format='(F3.1)')=='3.0 kg.m.s-2'
print "(A,L1)", '1 * force(float32) + 2 * force(float64) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(1)

call force%unset
force = force2 + force1
test_passed(2) = force%stringify(format='(F3.1)')=='3.0 kg.m.s-2'
print "(A,L1)", '2 * force(float64) + 1 * force(float32) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(2)

call force%unset
force = force1 + force3
test_passed(3) = force%stringify(format='(F3.1)')=='4.0 kg.m.s-2'
print "(A,L1)", '1 * force(float32) + 3 * force(float128) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(3)

call force%unset
force = force3 + force1
test_passed(4) = force%stringify(format='(F3.1)')=='4.0 kg.m.s-2'
print "(A,L1)", '3 * force(float128) + 1 * force(float32) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(4)

call force%unset
force = force2 + force3
test_passed(5) = force%stringify(format='(F3.1)')=='5.0 kg.m.s-2'
print "(A,L1)", '2 * force(float64) + 3 * force(float128) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(5)

call force%unset
force = force3 + force2
test_passed(6) = force%stringify(format='(F3.1)')=='5.0 kg.m.s-2'
print "(A,L1)", '3 * force(float164) + 2 * force(float32) = '//force%stringify(format='(F3.1)')//', is correct? ', test_passed(6)

print "(A,L1)", new_line('a')//'Are all tests passed? ', all(test_passed)
stop
!-----------------------------------------------------------------------------------------------------------------------------------
endprogram fury_test_qreal_add_mixed
